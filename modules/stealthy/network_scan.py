...
_PROBE_PROFILES = [
    # (label, data_flag, payload)
    ("HTTP", "--data-string", "GET / HTTP/1.1\r\nHost: www.microsoft.com\r\nAccept: */*\r\n\r\n"),
    ("DNS",  "--data",        _DNS_PAYLOAD),
    ("NTP",  "--data",        _NTP_PAYLOAD),
]
...
def probe_encapsulation_flags():
    """Return nmap data-payload flags that disguise the probe as HTTP, DNS, or NTP (Linux only)."""
    if sys.platform == "win32":
        return []
    label, data_flag, payload = random.choice(_PROBE_PROFILES)
    debug_print(f" [*] Probe disguised as {label}")
